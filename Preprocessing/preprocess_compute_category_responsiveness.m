...
for si = 1:length(listing)
    ...
    stimuli_subset = s.stimgroups ==  category;
    s.stimseq = s.stimseq(stimuli_subset);
    s.stimgroups = s.stimgroups(stimuli_subset);
    s.data = s.data(stimuli_subset, :, :);
    ...
    clearvars -except freqlist windows indata outdata nfreqs nwin listing results category
    fprintf('\n');
